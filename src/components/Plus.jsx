...
export default function Plus({ showInfos, hideInfos, hideValue }) {
  ...
  useEffect(() => {
    loader.load(
      "./models/plus.glb",
      (d) => {
        sceneRef.current.position.x = 9.5;
  ...
  const handleClick = (num) => {
    const position = {
      1: {
        cameraPos: [21.19, 7.07, -3.48],
        target: [21.18, 9.25, -3.48],
      },
    };
  ...
  return (
    <mesh
      onClick={(e) => {
        setTimeout(() => {
          document.getElementById("more-infos").style.display = "flex";
        }, 400);
